...
void region(int* width, int* height, int **mat)
{
    int cols = (*width);
    int rows = (*height);
    ...
    int prev_sum = 0;
    int c, r;
    int left_found = 0, top_found = 0;

// Find summed rows and columns
    for (c = 0; c < cols; c++)
    {

        for (r = 0; r < rows; r++)
        {
            row_sum[c] = row_sum[c] + mat[r][c];
            col_sum[r] = col_sum[r] + mat[r][c];
        } // for all rows
...
// Finding left and right edges
    ...
    for (c = 0; c < cols; c++)
    {
        ...
        if (!left_found)
            ...
            if (prev_sum * .75 >= col_sum[c])
                xLeft = c;
        else // left_found

            if (prev_sum * 1.5 <= col_sum[c])
                xRight = c;
...
// Finding top and bottom edges
    ...
    for (r = 0; r < rows; r++)
    {
        ...
        if (!top_found)
            ...
            if (prev_sum * .75 >= row_sum[r])
                xTop[] = r;
        else // top_found

            if (prev_sum * 1.5 <= row_sum[r])
                xBottom = r;
